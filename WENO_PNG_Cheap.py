...
-------
  ...
  Four-quadrant initial condition with discontinuities along x=1 and y=1.
  ...
  BC : transmissive (zero-order extrapolation) on all four walls.

Usage
-----
  python WENO_png_riemann.py            # GPU (CUDA) if available
  python WENO_png_riemann.py --cpu      # force CPU
"""
...
# ══════════════════════════════════════════════════════════════════════════════
#  §5  Velocity derivatives + Ducros sensor
# ══════════════════════════════════════════════════════════════════════════════
...
def wcns_x_kernel(
    ...
    if is_smooth:
        eta = F0p6
        ul0=central6_ul(cons[face_ix-2,iy,0],cons[face_ix-1,iy,0],cons[face_ix,iy,0],cons[face_ix+1,iy,0],cons[face_ix+2,iy,0],cons[face_ix+3,iy,0],eta)
        ur0=central6_ur(cons[face_ix-2,iy,0],cons[face_ix-1,iy,0],cons[face_ix,iy,0],cons[face_ix+1,iy,0],cons[face_ix+2,iy,0],cons[face_ix+3,iy,0],eta)
        ul1=central6_ul(cons[face_ix-2,iy,1],cons[face_ix-1,iy,1],cons[face_ix,iy,1],cons[face_ix+1,iy,1],cons[face_ix+2,iy,1],cons[face_ix+3,iy,1],eta)
        ur1=central6_ur(cons[face_ix-2,iy,1],cons[face_ix-1,iy,1],cons[face_ix,iy,1],cons[face_ix+1,iy,1],cons[face_ix+2,iy,1],cons[face_ix+3,iy,1],eta)
        ul2=central6_ul(cons[face_ix-2,iy,2],cons[face_ix-1,iy,2],cons[face_ix,iy,2],cons[face_ix+1,iy,2],cons[face_ix+2,iy,2],cons[face_ix+3,iy,2],HALF)
        ur2=central6_ur(cons[face_ix-2,iy,2],cons[face_ix-1,iy,2],cons[face_ix,iy,2],cons[face_ix+1,iy,2],cons[face_ix+2,iy,2],cons[face_ix+3,iy,2],HALF)
        ul3=central6_ul(cons[face_ix-2,iy,3],cons[face_ix-1,iy,3],cons[face_ix,iy,3],cons[face_ix+1,iy,3],cons[face_ix+2,iy,3],cons[face_ix+3,iy,3],eta)
        ur3=central6_ur(cons[face_ix-2,iy,3],cons[face_ix-1,iy,3],cons[face_ix,iy,3],cons[face_ix+1,iy,3],cons[face_ix+2,iy,3],cons[face_ix+3,iy,3],eta)
        e_m2=char_transform(cons,face_ix-2,iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        e_m1=char_transform(cons,face_ix-1,iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        e_0 =char_transform(cons,face_ix,  iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        e_1 =char_transform(cons,face_ix+1,iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        e_2 =char_transform(cons,face_ix+2,iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        e_3 =char_transform(cons,face_ix+3,iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        centL=weno5_ul(e_m2,e_m1,e_0,e_1,e_2); centR=weno5_ur(e_m1,e_0,e_1,e_2,e_3)
        le0=F1-GM1*HALF*q2v/c2; le1=GM1*u_r/c2; le2=GM1*v_r/c2; le3=NF1*GM1/c2
        dL=centL-(le0*ul0+le1*ul1+le2*ul2+le3*ul3)
        dR=centR-(le0*ur0+le1*ur1+le2*ur2+le3*ur3)
        consl[face_ix,iy,0]=ul0+dL;         consr[face_ix,iy,0]=ur0+dR
        consl[face_ix,iy,1]=ul1+dL*u_r;     consr[face_ix,iy,1]=ur1+dR*u_r
        consl[face_ix,iy,2]=ul2+dL*v_r;     consr[face_ix,iy,2]=ur2+dR*v_r
        consl[face_ix,iy,3]=ul3+dL*HALF*q2v; consr[face_ix,iy,3]=ur3+dR*HALF*q2v
    else:
        c_m2=char_transform(cons,face_ix-2,iy,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_m1=char_transform(cons,face_ix-1,iy,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_0 =char_transform(cons,face_ix,  iy,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_1 =char_transform(cons,face_ix+1,iy,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_2 =char_transform(cons,face_ix+2,iy,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_3 =char_transform(cons,face_ix+3,iy,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        ul0=weno5_ul(c_m2,c_m1,c_0,c_1,c_2); ur0=weno5_ur(c_m1,c_0,c_1,c_2,c_3)
        c_m2=char_transform(cons,face_ix-2,iy,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_m1=char_transform(cons,face_ix-1,iy,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_0 =char_transform(cons,face_ix,  iy,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_1 =char_transform(cons,face_ix+1,iy,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_2 =char_transform(cons,face_ix+2,iy,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_3 =char_transform(cons,face_ix+3,iy,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        ul1=weno5_ul(c_m2,c_m1,c_0,c_1,c_2); ur1=weno5_ur(c_m1,c_0,c_1,c_2,c_3)
        c_m2=char_transform(cons,face_ix-2,iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_m1=char_transform(cons,face_ix-1,iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_0 =char_transform(cons,face_ix,  iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_1 =char_transform(cons,face_ix+1,iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_2 =char_transform(cons,face_ix+2,iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_3 =char_transform(cons,face_ix+3,iy,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        ul2=weno5_ul(c_m2,c_m1,c_0,c_1,c_2); ur2=weno5_ur(c_m1,c_0,c_1,c_2,c_3)
        c_m2=char_transform(cons,face_ix-2,iy,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_m1=char_transform(cons,face_ix-1,iy,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_0 =char_transform(cons,face_ix,  iy,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_1 =char_transform(cons,face_ix+1,iy,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_2 =char_transform(cons,face_ix+2,iy,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_3 =char_transform(cons,face_ix+3,iy,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        ul3=weno5_ul(c_m2,c_m1,c_0,c_1,c_2); ur3=weno5_ur(c_m1,c_0,c_1,c_2,c_3)
        consl[face_ix,iy,0]=Rv0(ul0,ul1,ul2,ul3);               consr[face_ix,iy,0]=Rv0(ur0,ur1,ur2,ur3)
        consl[face_ix,iy,1]=Rv1(ul0,ul1,ul2,ul3,u_r,c_r,mx,lx); consr[face_ix,iy,1]=Rv1(ur0,ur1,ur2,ur3,u_r,c_r,mx,lx)
        consl[face_ix,iy,2]=Rv2(ul0,ul1,ul2,ul3,v_r,c_r,my,ly); consr[face_ix,iy,2]=Rv2(ur0,ur1,ur2,ur3,v_r,c_r,my,ly)
        consl[face_ix,iy,3]=Rv3(ul0,ul1,ul2,ul3,H_r,qn,c_r,ql,q2v); consr[face_ix,iy,3]=Rv3(ur0,ur1,ur2,ur3,H_r,qn,c_r,ql,q2v)


# ══════════════════════════════════════════════════════════════════════════════
#  §7  WENO reconstruction — Y
# ══════════════════════════════════════════════════════════════════════════════
...
def wcns_y_kernel(
    ...
    if is_smooth:
        eta = F0p6
        ul0=central6_ul(cons[ix,face_iy-2,0],cons[ix,face_iy-1,0],cons[ix,face_iy,0],cons[ix,face_iy+1,0],cons[ix,face_iy+2,0],cons[ix,face_iy+3,0],eta)
        ur0=central6_ur(cons[ix,face_iy-2,0],cons[ix,face_iy-1,0],cons[ix,face_iy,0],cons[ix,face_iy+1,0],cons[ix,face_iy+2,0],cons[ix,face_iy+3,0],eta)
        ul1=central6_ul(cons[ix,face_iy-2,1],cons[ix,face_iy-1,1],cons[ix,face_iy,1],cons[ix,face_iy+1,1],cons[ix,face_iy+2,1],cons[ix,face_iy+3,1],HALF)
        ur1=central6_ur(cons[ix,face_iy-2,1],cons[ix,face_iy-1,1],cons[ix,face_iy,1],cons[ix,face_iy+1,1],cons[ix,face_iy+2,1],cons[ix,face_iy+3,1],HALF)
        ul2=central6_ul(cons[ix,face_iy-2,2],cons[ix,face_iy-1,2],cons[ix,face_iy,2],cons[ix,face_iy+1,2],cons[ix,face_iy+2,2],cons[ix,face_iy+3,2],eta)
        ur2=central6_ur(cons[ix,face_iy-2,2],cons[ix,face_iy-1,2],cons[ix,face_iy,2],cons[ix,face_iy+1,2],cons[ix,face_iy+2,2],cons[ix,face_iy+3,2],eta)
        ul3=central6_ul(cons[ix,face_iy-2,3],cons[ix,face_iy-1,3],cons[ix,face_iy,3],cons[ix,face_iy+1,3],cons[ix,face_iy+2,3],cons[ix,face_iy+3,3],eta)
        ur3=central6_ur(cons[ix,face_iy-2,3],cons[ix,face_iy-1,3],cons[ix,face_iy,3],cons[ix,face_iy+1,3],cons[ix,face_iy+2,3],cons[ix,face_iy+3,3],eta)
        e_m2=char_transform(cons,ix,face_iy-2,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        e_m1=char_transform(cons,ix,face_iy-1,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        e_0 =char_transform(cons,ix,face_iy,  2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        e_1 =char_transform(cons,ix,face_iy+1,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        e_2 =char_transform(cons,ix,face_iy+2,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        e_3 =char_transform(cons,ix,face_iy+3,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        centL=weno5_ul(e_m2,e_m1,e_0,e_1,e_2); centR=weno5_ur(e_m1,e_0,e_1,e_2,e_3)
        le0=F1-GM1*HALF*q2v/c2; le1=GM1*u_r/c2; le2=GM1*v_r/c2; le3=NF1*GM1/c2
        dL=centL-(le0*ul0+le1*ul1+le2*ul2+le3*ul3)
        dR=centR-(le0*ur0+le1*ur1+le2*ur2+le3*ur3)
        consl[ix,face_iy,0]=ul0+dL;          consr[ix,face_iy,0]=ur0+dR
        consl[ix,face_iy,1]=ul1+dL*u_r;      consr[ix,face_iy,1]=ur1+dR*u_r
        consl[ix,face_iy,2]=ul2+dL*v_r;      consr[ix,face_iy,2]=ur2+dR*v_r
        consl[ix,face_iy,3]=ul3+dL*HALF*q2v; consr[ix,face_iy,3]=ur3+dR*HALF*q2v
    else:
        c_m2=char_transform(cons,ix,face_iy-2,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_m1=char_transform(cons,ix,face_iy-1,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_0 =char_transform(cons,ix,face_iy,  0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_1 =char_transform(cons,ix,face_iy+1,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_2 =char_transform(cons,ix,face_iy+2,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_3 =char_transform(cons,ix,face_iy+3,0,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        ul0=weno5_ul(c_m2,c_m1,c_0,c_1,c_2); ur0=weno5_ur(c_m1,c_0,c_1,c_2,c_3)
        c_m2=char_transform(cons,ix,face_iy-2,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_m1=char_transform(cons,ix,face_iy-1,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_0 =char_transform(cons,ix,face_iy,  1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_1 =char_transform(cons,ix,face_iy+1,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_2 =char_transform(cons,ix,face_iy+2,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_3 =char_transform(cons,ix,face_iy+3,1,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        ul1=weno5_ul(c_m2,c_m1,c_0,c_1,c_2); ur1=weno5_ur(c_m1,c_0,c_1,c_2,c_3)
        c_m2=char_transform(cons,ix,face_iy-2,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_m1=char_transform(cons,ix,face_iy-1,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_0 =char_transform(cons,ix,face_iy,  2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_1 =char_transform(cons,ix,face_iy+1,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_2 =char_transform(cons,ix,face_iy+2,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_3 =char_transform(cons,ix,face_iy+3,2,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        ul2=weno5_ul(c_m2,c_m1,c_0,c_1,c_2); ur2=weno5_ur(c_m1,c_0,c_1,c_2,c_3)
        c_m2=char_transform(cons,ix,face_iy-2,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_m1=char_transform(cons,ix,face_iy-1,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_0 =char_transform(cons,ix,face_iy,  3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_1 =char_transform(cons,ix,face_iy+1,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_2 =char_transform(cons,ix,face_iy+2,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        c_3 =char_transform(cons,ix,face_iy+3,3,u_r,v_r,c_r,c2,mx,my,qn,q2v,lx,ly,ql)
        ul3=weno5_ul(c_m2,c_m1,c_0,c_1,c_2); ur3=weno5_ur(c_m1,c_0,c_1,c_2,c_3)
        consl[ix,face_iy,0]=Rv0(ul0,ul1,ul2,ul3);               consr[ix,face_iy,0]=Rv0(ur0,ur1,ur2,ur3)
        consl[ix,face_iy,1]=Rv1(ul0,ul1,ul2,ul3,u_r,c_r,mx,lx); consr[ix,face_iy,1]=Rv1(ur0,ur1,ur2,ur3,u_r,c_r,mx,lx)
        consl[ix,face_iy,2]=Rv2(ul0,ul1,ul2,ul3,v_r,c_r,my,ly); consr[ix,face_iy,2]=Rv2(ur0,ur1,ur2,ur3,v_r,c_r,my,ly)
        consl[ix,face_iy,3]=Rv3(ul0,ul1,ul2,ul3,H_r,qn,c_r,ql,q2v); consr[ix,face_iy,3]=Rv3(ur0,ur1,ur2,ur3,H_r,qn,c_r,ql,q2v)


# ══════════════════════════════════════════════════════════════════════════════
#  §8  HLLC flux + residual — X
# ══════════════════════════════════════════════════════════════════════════════
...
def save_png(rho_np, x_np, y_np, step, time_sim):
    ...
    ax.contour(xg, yg, d, 36, linewidths=0.5, colors='k')
...
def main():
    print(f"2D Euler/Warp  Riemann Config-3  NX=NY={NX}  device={DEVICE}")
